---
title: "AI Agent Frameworks (2026 Update): 8 SDKs Compared + the ..."
...
Reserved B200 capacity by the GPU-hour.
...
8 AI agent frameworks compared with exact 2026 releases: Claude Agent SDK, OpenAI Agents SDK, Google ADK, LangGraph, CrewAI (52.4k stars), Smolagents, Pydantic AI, Microsoft Agent Framework 1.0. Plus the full Claude Agent SDK reference: query() primitive, lifecycle hooks, subagents, schema, streaming, isolation, token handling, MCP, and A2A.

![AI Agent Frameworks (2026 Update): 8 SDKs Compared + the Claude Agent SDK Primitive Reference](/_next/image?url=%2Fimages%2Fseo%2Fai-agent-framework.png&w=3840&q=75)
...
Built-in tools include `Read`, `Write`, `Edit`, `Bash`, `Glob`, `Grep`, `WebSearch`, `WebFetch`, and `AskUserQuestion`. You scope what the agent may call through `allowedTools`.
...
`hooks={"PostToolUse": [HookMatcher(matcher="Edit|Write", hooks=[fn])]}`

Matcher syntax mirrors Claude Code: `"*"` or omitted matches all; letters, digits, underscores, and pipes are an exact or pipe-separated list (`"Edit|Write"`); anything else is treated as a JavaScript regex (`"mcp__memory__.*"`). MCP tools are named `mcp__<server>__<tool>`.
...
Morph is framework-agnostic. Point Claude Agent SDK, LangGraph, CrewAI, or OpenAI Agents SDK at the endpoint and the API does not care which framework dispatched the request. See [Models](/products/models) and [Pricing](/pricing) for the full catalog.
